...
static void read_ciao(
		...
		hls::stream<int> &hub_stream,
		hls::stream<int> &hub_bypass_stream,
		const int level
		)
{
	...
	while((rpao_empty != 1) || (done != 1)){
		...
		if(!hub_stream.empty())	{
			hub_bypass_stream << hub_stream.read();
		}
		if(rpao_empty != 1){
...
/*	if(!level){	
	...
	while ((frontier_stream.empty() != 1)){
			int frontier_vidx = frontier_stream.read();
		 	int hash_idx = frontier_vidx & HASH_MASK;
			frontier_table[hash_idx] = frontier_vidx;
			}
			*/
...
#pragma HLS pipeline
		...
		if(!frontier_stream.empty()){
			int vidx = frontier_stream.read();
			int14_dt hash_idx = vidx & HASH_MASK;
			int hash_val = frontier_table[hash_idx];
			frontier_table[hash_idx] = hash_val;
		}
...
*/
	...
	hls::stream<int> hub_bypass_stream0;	
	hls::stream<int> hub_bypass_stream1;
	hls::stream<int> hub_bypass_stream2;
	hls::stream<int> hub_bypass_stream3;
...
#pragma HLS STREAM variable=hub_bypass_stream0 depth=64
#pragma HLS STREAM variable=hub_bypass_stream1 depth=64
#pragma HLS STREAM variable=hub_bypass_stream2 depth=64
#pragma HLS STREAM variable=hub_bypass_stream3 depth=64
...
#pragma HLS dataflow
	...
	read_rpao(rpao0, frontier_stream0, rpao_stream0, rpao_done_stream0, hub_stream0,block);
	read_rpao(rpao1, frontier_stream1, rpao_stream1, rpao_done_stream1, hub_stream1,block);
	read_rpao(rpao2, frontier_stream2, rpao_stream2, rpao_done_stream2, hub_stream2,block);
	read_rpao(rpao3, frontier_stream3, rpao_stream3, rpao_done_stream3, hub_stream3,block);

	read_ciao(ciao0, rpao_stream0, rpao_done_stream0, ciao_stream0, 
			ciao_done_stream0,hub_stream0, hub_bypass_stream0, level);
	read_ciao(ciao1, rpao_stream1, rpao_done_stream1, ciao_stream1, 
			ciao_done_stream1,hub_stream1, hub_bypass_stream1, level);
	read_ciao(ciao2, rpao_stream2, rpao_done_stream2, ciao_stream2, 
			ciao_done_stream2,hub_stream2, hub_bypass_stream2, level);
	read_ciao(ciao3, rpao_stream3, rpao_done_stream3, ciao_stream3, 
			ciao_done_stream3, hub_stream3, hub_bypass_stream3, level);

    update_depth(depth_for_expand0, hub_bypass_stream0, ciao_stream0, ciao_done_stream0, level);
    update_depth(depth_for_expand1, hub_bypass_stream1, ciao_stream1, ciao_done_stream1, level);
    update_depth(depth_for_expand2, hub_bypass_stream2, ciao_stream2, ciao_done_stream2, level);
    update_depth(depth_for_expand3, hub_bypass_stream3, ciao_stream3, ciao_done_stream3, level);
}
}
