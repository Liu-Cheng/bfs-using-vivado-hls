...
#define LEN6_WIDTH 6
#define INDEX6_WIDTH 12
#define TAG6_WIDTH 14
#define INIT6_VAL 0x3FFF
#define CACHE6_SIZE 4096
		...
		const char level)
{
	...
	bool cache0_dirty[CACHE6_SIZE];
	bool cache1_dirty[CACHE6_SIZE];
	tag6_dt cache0_tag[CACHE6_SIZE];
	tag6_dt cache1_tag[CACHE6_SIZE];
	int512_dt cache0_data[CACHE6_SIZE];
	int512_dt cache1_data[CACHE6_SIZE];
	...
	if(level_plus1 == 1){
        ...
        #pragma HLS pipeline
			cache0_tag[i] = INIT6_VAL;
			cache0_dirty[i] = false;
			cache1_tag[i] = INIT6_VAL;
			cache1_dirty[i] = false;
		}
#ifndef SW_EMU
	}
#endif
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			tag6_dt old_tag0 = cache0_tag[req_idx];
			tag6_dt old_tag1 = cache1_tag[req_idx];
			int512_dt cache0_line = cache0_data[req_idx];
			int512_dt cache1_line = cache1_data[req_idx];
			uint32_dt req_line_addr = vidx >> LEN6_WIDTH;
			uint26_dt cache0_line_addr = (old_tag0, req_idx);
			uint26_dt cache1_line_addr = (old_tag1, req_idx);
