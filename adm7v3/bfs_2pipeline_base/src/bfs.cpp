...
static void read_depth(
        ...
        const int part_size)
{
    read_depth: for (int i = 0; i < part_size; i++){
#pragma HLS pipeline
        depth_inspect_stream << depth[start_idx + i];
    }
}
		...
		const char level)
{
	char data; 
	int count = 0;
    inspect: for (int i = 0; i < part_size; i++){
#pragma HLS pipeline
		data = depth_inspect_stream.read();
		if(data == level){
			frontier_stream << (start_idx + i);
			count++;
		}
...
#pragma HLS STREAM variable=ngb_done_stream1 depth=4
	...
	int part = vertex_num/2;
	int last_part = vertex_num - part;
#pragma HLS dataflow
    read_depth(depth_for_inspect0, depth_inspect_stream0, 0, part);
    read_depth(depth_for_inspect1, depth_inspect_stream1, part, last_part);

	frontier_inspect(depth_inspect_stream0, inspect_done_stream0, 
			frontier_stream0, 0, part, &frontier_size0[0], level);
	frontier_inspect(depth_inspect_stream1, inspect_done_stream1, 
			frontier_stream1, part, last_part, &frontier_size1[1], level);
