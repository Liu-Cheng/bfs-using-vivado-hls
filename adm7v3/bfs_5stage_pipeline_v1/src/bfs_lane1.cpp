...
{
    read_depth: 
	for (int i = 0; i < vertex_num; i += STRIDE_LEN){
		for(int j = 0; j < BURST_LEN; j++){
        #pragma HLS pipeline
			depth_inspect_stream << depth_for_inspect[i + j + BURST_LEN];
		}
    }
}
		...
		const int level)
{
	...
	for (int i = 0; i < vertex_num; i += STRIDE_LEN){
        ...
        #pragma HLS pipeline
			...
			if(d == level){
				frontier_stream << i + j + BURST_LEN;
				if(i+j+BURST_LEN==46680) printf("46680 frontier is found.\n");
			}
		}
	}
...
bfs_stage1(depth_inspect_stream, inspect_done_stream, frontier_stream, vertex_num, level);
