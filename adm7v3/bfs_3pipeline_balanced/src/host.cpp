...
void swBfsInit(CSR* csr, char* depth, const int &vertexIdx){
...
void swBfs(CSR* csr, char* depth, const int &vertexIdx){
	...
	while(true){
		...
		level++;
		std::cout << "sw frontier size = " << frontier.size() << std::endl;
		frontier.clear();
	}
}
...
std::string gName = "rmat-21-32";
	...
	int* frontierSize = (int*)malloc(thread_num * sizeof(int));	

	int ciaoSize = csr->ciao.size();
	int rpaoSize = csr->rpao.size();

	std::cout << "soft bfs starts." << std::endl;
	swBfsInit(csr, swDepth, startVertexIdx);
	begin = clock();
	...
	cl_mem devMemFrontierSize = xcl_malloc(world, CL_MEM_WRITE_ONLY, thread_num * sizeof(int));
	...
	while(sum > 0){
		...
		xcl_memcpy_from_device(world, frontierSize, devMemFrontierSize, thread_num * sizeof(int));
		sum = frontierSize[0] + frontierSize[1] + frontierSize[2];
