...
#define HASH_ID_WIDTH 17
#define HASH_TAG_WIDTH 15
#define HASH_TAG_INIT_VAL 0x7FFF
...
#define DEPTH_CACHE_SIZE (16384*2)
#define DEPTH_OFFSET_WIDTH 6
#define DEPTH_INDEX_WIDTH 15
#define DEPTH_TAG_WIDTH 11
#define DEPTH_TAG_INIT_VAL 0x7FF
...
//#pragma HLS DEPENDENCE variable=cache_tag inter false
	...
	hit_sum += hit_num;
	if(read_num > 0 && read_sum > 0){ 
		printf("cache read hit rate %3.2f\n", hit_num * 1.0 / read_num);
		printf("total cache read hit rate %3.2f \n", hit_sum * 1.0 / read_sum);
	}
#endif
}
...
//#pragma HLS DEPENDENCE variable=cache_dirty inter false
	...
	hit_sum += hit_num;
	write_sum += write_num;
	if(hit_num > 0 && write_sum > 0){
		printf("write cache hit rate %3.2f\n%", hit_num * 1.0/write_num);
		printf("total cache write hit rate %3.2f\n", hit_sum * 1.0/write_sum);
	}
#endif
