		...
		)
{
	...
	int512_dt cache_data_set0[1024];
	int512_dt cache_data_set1[1024];
	uint18_dt cache_tag_set0[1024];
	uint18_dt cache_tag_set1[1024];
	uint1_dt cache_pri[1024]; // determine which to replace
	...
	if(level == 0){
        ...
        #pragma HLS pipeline
			cache_tag_set0[i] = 0x3FFF;
			cache_tag_set1[i] = 0x3FFF;
			cache_pri[i] = 0;
		}
#ifndef SW_EMU
	}
	...
	while((rpao_empty != 1) || (done != 1)){
		...
		if(rpao_empty != 1){
            ...
            #pragma HLS pipeline
				uint32_dt addr = i.range(31,4);
				uint4_dt offset = i.range(3 ,0);
				uint10_dt index = i.range(13, 4);
				uint18_dt tag = i.range(31, 14);
				uint1_dt _pri = cache_pri[index];
				uint18_dt _tag_set0 = cache_tag_set0[index];
				uint18_dt _tag_set1 = cache_tag_set1[index];
#ifdef SW_ANALYSIS
				ciao_cnt++;
#endif
...
static void bfs_stage4(
		...
		const char level)
{
	// Cache data structure 6 bit offset, 10 bit index, 16bit tag
	bool cache_dirty[1024];
	uint16_dt cache_tag[1024];
	int512_dt cache_data[1024];
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			uint10_dt req_idx = vidx.range(15, 6);
			uint16_dt req_tag = vidx.range(31, 16);
			uint16_dt old_tag = cache_tag[req_idx];
			int512_dt cache_line = cache_data[req_idx];
			uint32_dt req_line_addr = vidx >> 6;
    ...
    #pragma HLS pipeline
		...
		if(dirty == true){
			...
			cache_line_addr.range(9, 0) = i;
			cache_line_addr.range(25, 10) = cache_tag[i];
