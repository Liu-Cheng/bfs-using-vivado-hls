...
#define DEG_HLIMIT 512
#define DEG_LLIMIT 16
		...
		)
{
	...
	tag_dt visited_table0[HASH_SIZE];
	tag_dt visited_table1[HASH_SIZE];
	tag_dt visited_table2[HASH_SIZE];
	tag_dt visited_table3[HASH_SIZE];
	...
	uint2_dt sel = 0;
  	...
  	if(level == 0){
        ...
        #pragma HLS pipeline
			...
			visited_table0[i] = 0x3FFFF;
			visited_table1[i] = 0x3FFFF;
			visited_table2[i] = 0x3FFFF;
			visited_table3[i] = 0x3FFFF;
		}
    ...
    #pragma HLS pipeline
		...
		if(visited_stream.empty() != 1){
			uint32_dt vidx = visited_stream.read();
			index_dt hash_idx = vidx.range(HASH_ID_WIDTH-1, 0);
			tag_dt hash_tag = vidx.range(31, HASH_ID_WIDTH);
			if(sel == 0) {
				visited_table0[hash_idx] = hash_tag;
				sel = 1;
			}
			else if(sel == 1) {
				visited_table1[hash_idx] = hash_tag;
				sel = 2;
			}
			else if(sel == 2) { 
				visited_table2[hash_idx] = hash_tag;
				sel = 3;
			}
			else if(sel == 3) {
				visited_table3[hash_idx] = hash_tag;
				sel = 0;
			}
		}
		...
		if(ciao_empty != 1){
			...
			tag_dt visited_val0 = visited_table0[hash_idx];
			tag_dt visited_val1 = visited_table1[hash_idx];
			tag_dt visited_val2 = visited_table2[hash_idx];
			tag_dt visited_val3 = visited_table3[hash_idx];

			if(visited_val0 != hash_tag && visited_val1 != hash_tag 
					&& visited_val2 != hash_tag && visited_val3 != hash_tag
					&& hash_val0 != hash_tag && hash_val1 != hash_tag 
...
#pragma HLS STREAM variable=visited_stream depth=32
#pragma HLS STREAM variable=frontier_stream depth=32
...
bfs_stage2(rpao, frontier_stream, inspect_done_stream, rpao_stream, rpao_done_stream, visited_stream);
bfs_stage3(ciao, rpao_stream, rpao_done_stream, ciao_stream, ciao_done_stream);
bfs_squeeze_ciao(ciao_stream, ciao_done_stream, 
		ciao_squeeze_stream, ciao_squeeze_done_stream, visited_stream, level);
