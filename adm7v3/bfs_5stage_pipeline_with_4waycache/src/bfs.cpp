...
//#define CIAO_ONE_DATA_CACHE_ANALYSIS
		...
		)
{
...
#ifdef CIAO_ONE_DATA_CACHE_ANALYSIS
	int one_data_cache_hit_num = 0;
	int one_data_cache_ciao_num = 0;
	static int one_data_cache_hit_sum = 0;
	static int one_data_cache_ciao_sum = 0;
	uint32_dt last_vidx = 0xFFFFFFFF;
#endif
...
#ifdef CIAO_ONE_DATA_CACHE_ANALYSIS
					uint32_dt tmp = word_old.range((offset+1)*32 - 1, offset * 32);
					if(tmp == last_vidx){
						one_data_cache_hit_num++;
					}
					else{
						last_vidx = tmp;
					}
...
#ifdef CIAO_ONE_DATA_CACHE_ANALYSIS
					uint32_dt tmp = word.range((offset+1)*32 - 1, offset * 32);
					if(tmp == last_vidx){
						one_data_cache_hit_num++;
					}
					else{
	...
	hit_sum += hit_num;
	ciao_sum += ciao_num;
	if(ciao_num > 0 && ciao_sum > 0){
		printf("Level = %d, hit_num = %d, ciao_sum =%d, ciao cache hit rate = %f\n", (int)level, hit_num, ciao_num, hit_num*1.0/ciao_num);
		printf("Total ciao cache hit rate = %f\n", hit_sum * 1.0/ciao_sum);
	}
#endif

#ifdef CIAO_ONE_DATA_CACHE_ANALYSIS
	one_data_cache_hit_sum += one_data_cache_hit_num;
	one_data_cache_ciao_sum += one_data_cache_ciao_num;
	if(one_data_cache_ciao_num > 0 && one_data_cache_ciao_sum > 0){
		printf("one data cache hit rate: %f\n", one_data_cache_hit_num * 1.0/one_data_cache_ciao_num);
		printf("one data cache overall hit rate: %f\n", one_data_cache_hit_sum * 1.0/one_data_cache_ciao_sum);
	}
#endif
		...
		)
{
	...
	if(ciao_num > 0){
		ciao_sum += ciao_num;
		hit_sum += hit_num;
		printf("hash table hit %d  %0.3f\n", hit_num, 100.0*hit_num/ciao_num);
...
{
	uint1_dt cache0_dirty[DEPTH_CACHE_SIZE];
	uint1_dt cache1_dirty[DEPTH_CACHE_SIZE];
	uint1_dt cache2_dirty[DEPTH_CACHE_SIZE];
	uint1_dt cache3_dirty[DEPTH_CACHE_SIZE];
	int512_dt cache0_data[DEPTH_CACHE_SIZE];
	int512_dt cache1_data[DEPTH_CACHE_SIZE];
	int512_dt cache2_data[DEPTH_CACHE_SIZE];
	int512_dt cache3_data[DEPTH_CACHE_SIZE];
	depth_tag_dt cache0_tag[DEPTH_CACHE_SIZE];
	depth_tag_dt cache1_tag[DEPTH_CACHE_SIZE];
	depth_tag_dt cache2_tag[DEPTH_CACHE_SIZE];
	depth_tag_dt cache3_tag[DEPTH_CACHE_SIZE];
	...
	if(level_plus1 == 1){
        ...
        #pragma HLS pipeline II=1
			cache0_tag[i] = DEPTH_TAG_INIT_VAL;
			cache1_tag[i] = DEPTH_TAG_INIT_VAL;
			cache2_tag[i] = DEPTH_TAG_INIT_VAL;
			cache3_tag[i] = DEPTH_TAG_INIT_VAL;
			cache0_dirty[i] = 0;
			cache1_dirty[i] = 0;
			cache2_dirty[i] = 0;
			cache3_dirty[i] = 0;
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			depth_tag_dt _tag0 = cache0_tag[index];
			depth_tag_dt _tag1 = cache1_tag[index];
			depth_tag_dt _tag2 = cache2_tag[index];
			depth_tag_dt _tag3 = cache3_tag[index];

			int512_dt _word0 = cache0_data[index];
			int512_dt _word1 = cache1_data[index];
			int512_dt _word2 = cache2_data[index];
			int512_dt _word3 = cache3_data[index];

			depth_addr_dt _addr0 = (_tag0, index);
			depth_addr_dt _addr1 = (_tag1, index);
			depth_addr_dt _addr2 = (_tag2, index);
			depth_addr_dt _addr3 = (_tag3, index);

			uint1_dt dirty0 = cache0_dirty[index];
			uint1_dt dirty1 = cache1_dirty[index];
			uint1_dt dirty2 = cache2_dirty[index];
			uint1_dt dirty3 = cache3_dirty[index];
			...
			if(_tag0 == tag){
				...
				read_hit_num++;
#endif
				char d = _word0.range(((offset+1)*8) - 1, (offset*8));
				if(d == -1){
#ifdef DEPTH_ANALYSIS
					write_hit_num++;
					write_num++;
#endif
					cache0_data[index].range(((offset+1)*8) - 1, (offset*8)) = level_plus1;
					cache0_dirty[index] = 1;
				}
			}
			else if(_tag1 == tag){
#ifdef DEPTH_ANALYSIS
				read_hit_num++;
#endif
				char d = _word1.range(((offset+1)*8) - 1, (offset*8));
				if(d == -1){
#ifdef DEPTH_ANALYSIS
					write_hit_num++;
					write_num++;
#endif
					cache1_data[index].range(((offset+1)*8) - 1, (offset*8)) = level_plus1;
					cache1_dirty[index] = 1;
				}
				...
				read_hit_num++;
#endif
				char d = _word2.range(((offset+1)*8) - 1, (offset*8));
				if(d == -1){
#ifdef DEPTH_ANALYSIS
					write_hit_num++;
					write_num++;
#endif
					cache2_data[index].range(((offset+1)*8) - 1, (offset*8)) = level_plus1;
					cache2_dirty[index] = 1;
				}
				...
				read_hit_num++;
#endif
				char d = _word3.range(((offset+1)*8) - 1, (offset*8));
				if(d == -1){
#ifdef DEPTH_ANALYSIS
					write_hit_num++;
					write_num++;
#endif
					cache3_data[index].range(((offset+1)*8) - 1, (offset*8)) = level_plus1;
					cache3_dirty[index] = 1;
				}
			...
			// cache miss
				...
				if(dirty0 == 1 && dirty1 == 1 && dirty2 == 1 && dirty3 == 1){
					if(cache_pri == 0){
						depth_for_update[_addr0] = _word0;
						cache_pri = 1;
					}
					else if(cache_pri == 1){
						depth_for_update[_addr1] = _word1;
						cache_pri = 2;
					}
					else if(cache_pri == 2){
						depth_for_update[_addr2] = _word2;
						cache_pri = 3;
					}
					else{
						depth_for_update[_addr3] = _word3;
						cache_pri = 0;
					}
				}
				...
				if(d == -1) write_num++;
#endif
				uint1_dt all_cache_clean = (dirty0 == 0) && (dirty1 == 0) && (dirty2 == 0) && (dirty3 == 0);
				if(dirty0 == 0 || (all_cache_clean && cache_pri == 0)){
					cache0_dirty[index] = (d == -1) ? 1 : 0;
					cache0_tag[index] = tag;
					cache0_data[index] = word;
					cache_pri = 1;
				}
				else if(dirty1 == 0 || (all_cache_clean && cache_pri == 1)){
					cache1_dirty[index] = (d == -1) ? 1 : 0;
					cache1_tag[index] = tag;
					cache1_data[index] = word;
					cache_pri = 2;
				}
				else if(dirty2 == 0 || (all_cache_clean && cache_pri == 2)){
					cache2_dirty[index] = (d == -1) ? 1 : 0;
					cache2_tag[index] = tag;
					cache2_data[index] = word;
					cache_pri = 3;
				}
				else if(dirty3 == 0 || (all_cache_clean && cache_pri == 3)){
					cache3_dirty[index] = (d == -1) ? 1 : 0;
					cache3_tag[index] = tag;
					cache3_data[index] = word;
					cache_pri = 0;
				}
    ...
    #pragma HLS pipeline
	   uint1_dt dirty0 = cache0_dirty[i];
	   uint1_dt dirty1 = cache1_dirty[i];
	   uint1_dt dirty2 = cache2_dirty[i];
	   uint1_dt dirty3 = cache3_dirty[i];
		if(dirty0){
			depth_addr_dt line_addr = (cache0_tag[i], i.range(DEPTH_INDEX_WIDTH - 1, 0));
			depth_for_update[line_addr] = cache0_data[i];
			cache0_dirty[i] = 0;
		}
		if(dirty1){
			depth_addr_dt line_addr = (cache1_tag[i], i.range(DEPTH_INDEX_WIDTH - 1, 0));
			depth_for_update[line_addr] = cache1_data[i];
			cache1_dirty[i] = 0;
		}
		if(dirty2){
			depth_addr_dt line_addr = (cache1_tag[i], i.range(DEPTH_INDEX_WIDTH - 1, 0));
			depth_for_update[line_addr] = cache2_data[i];
			cache2_dirty[i] = 0;
		}
		if(dirty3){
			depth_addr_dt line_addr = (cache1_tag[i], i.range(DEPTH_INDEX_WIDTH - 1, 0));
			depth_for_update[line_addr] = cache3_data[i];
			cache3_dirty[i] = 0;
		}
	}
