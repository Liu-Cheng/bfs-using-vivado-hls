...
static void frontier_inspect(
		hls::stream<int128_dt> &depth_inspect_stream, 
		hls::stream<int128_dt> &frontier_stream0,
		hls::stream<int128_dt> &frontier_stream1,
		hls::stream<int128_dt> &frontier_stream2,
		hls::stream<int128_dt> &frontier_stream3,
		int *frontier_size,
		int inspect_num,
		char level)
{
...
#pragma HLS pipeline	
		...
		for(int j = 0; j < 16; j++){
			bool b = data.range((j+1)*8-1, j * 8) == level;
			count += b;
			if(b){
				vidx[j] = (i << 4) + j;
			}
		...
		int128_dt v0, v1, v2, v3;
		v0.range(31, 0) = vidx[0];
		v0.range(63, 32) = vidx[1];
		v0.range(95, 64) = vidx[2];
		v0.range(127, 96) = vidx[3];

		v1.range(31, 0) = vidx[4];
		v1.range(63, 32) = vidx[5];
		v1.range(95, 64) = vidx[6];
		v1.range(127, 96) = vidx[7];

		v2.range(31, 0) = vidx[8];
		v2.range(63, 32) = vidx[9];
		v2.range(95, 64) = vidx[10];
		v2.range(127, 96) = vidx[11];

		v3.range(31, 0) = vidx[12];
		v3.range(63, 32) = vidx[13];
		v3.range(95, 64) = vidx[14];
		v3.range(127, 96) = vidx[15];
...
static void read_rpao(
		int *rpao,
		hls::stream<int128_dt> &frontier_stream,
		...
		int inspect_num)
{
	...
	for(int i = 0; i < inspect_num; i++){
		...
		for(int j = 0; j < 4; j++){
#pragma HLS pipeline
			v = vidx.range((j+1)*32-1, j*32);
			if(v >= 0){
				int64_dt rpitem;
				rpitem.range(31, 0) = rpao[v];
				rpitem.range(63, 32) = rpao[v+1];
				rpao_stream << rpitem;
			}
		}
...
#pragma HLS dataflow
	...
	read_rpao(rpao0, frontier_stream0, rpao_stream0, rpao_done_stream0, block);
	read_rpao(rpao1, frontier_stream1, rpao_stream1, rpao_done_stream1, block);
	read_rpao(rpao2, frontier_stream2, rpao_stream2, rpao_done_stream2, block);
	read_rpao(rpao3, frontier_stream3, rpao_stream3, rpao_done_stream3, block);
