...
#define HASH_ID_WIDTH 23
#define HASH_TAG_WIDTH (32 - HASH_ID_WIDTH)
#define HASH_TAG_INIT_VAL 0x1FF
		...
		)
{
	...
	tag_dt* hash_table = (tag_dt*)malloc(HASH_SIZE * sizeof(tag_dt));
	//tag_dt hash_table[HASH_SIZE];
#ifndef DEBUG
  	if(level == 0){
#endif
		for (int i = 0; i < HASH_SIZE; i++){
        #pragma HLS pipeline
			hash_table[i] = HASH_TAG_INIT_VAL;
		}
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			tag_dt hash_val = hash_table[hash_idx];

			if(hash_val != hash_tag){
				hash_table[hash_idx] = hash_tag;
