		...
		)
{
	...
	int512_dt cache_data_set0[CACHE_SIZE];
	int512_dt cache_data_set1[CACHE_SIZE];
	uint14_dt cache_tag_set0[CACHE_SIZE];
	uint14_dt cache_tag_set1[CACHE_SIZE];
	uint1_dt cache_pri[CACHE_SIZE]; // determine which to replace
	...
	while((rpao_empty != 1) || (done != 1)){
		...
		if(rpao_empty != 1){
            ...
            #pragma HLS pipeline
				...
				uint14_dt _tag_set0 = cache_tag_set0[index];
		...
		const char level)
{
	...
	cache_tag_dt cache_tag[CACHE_SIZE];
...
#endif
			uint32_dt vidx = ciao_stream.read();
			cache_offset_dt req_offset = vidx.range(5, 0);
			cache_index_dt req_idx = vidx.range(19, 6);
			cache_tag_dt req_tag = vidx.range(31, 20);
			cache_tag_dt old_tag = cache_tag[req_idx];
			int512_dt cache_line = cache_data[req_idx];
			uint32_dt req_line_addr = vidx >> LOG2_LEN;
    ...
    #pragma HLS pipeline
		...
		if(dirty == true){
			...
			cache_line_addr.range(13, 0) = i;
			cache_line_addr.range(25, 14) = cache_tag[i];
