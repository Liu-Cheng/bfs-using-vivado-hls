		...
		)
{
	...
	while((rpao_empty != 1) || (done != 1)){
		...
		if(rpao_empty != 1){
			...
			uint32_dt len = end - start;
			for(uint32_dt i = 0; i < len; i++){
			#pragma HLS LOOP_FLATTEN off
            #pragma HLS pipeline
				uint32_dt cindex = start + i;
				uint32_dt burst_addr = cindex >> 4;
				uint4_dt offset = cindex.range(3 ,0);
				uint14_dt index = cindex.range(17, 4);
				uint14_dt tag = cindex.range(31, 18);
...
bfs_stage4(depth_for_update, ciao_squeeze_stream, ciao_squeeze_done_stream, level_plus1);
