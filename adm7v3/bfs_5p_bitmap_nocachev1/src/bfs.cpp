...
#pragma HLS array_partition variable=bitmap cyclic factor=32 dim=1
...
//#pragma HLS DEPENDENCE variable=bitmap intra raw true
   	...
   	if(level == 0){
		...
		offset.range(11, 2) = root_vidx.range(17, 8);
		uint2_dt channel = root_vidx.range(7, 6);
		uint5_dt dim = root_vidx.range(22, 18);
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			uint2_dt channel = vidx.range(7, 6);
			if(channel != channel_idx) 
				printf("Vertex %d gets into the wrong lane %d.\n", (int)vidx, (int)channel_idx);
			uint12_dt offset;
			offset.range(1, 0) = vidx.range(5, 4);
			offset.range(11, 2)	= vidx.range(17, 8);
			uint5_dt dim = vidx.range(22, 18);
			uint16_dt mask = get_mask(n);
...
#pragma HLS STREAM variable=ciao_squeeze_stream0 depth=1024
#pragma HLS STREAM variable=ciao_squeeze_stream1 depth=1024
#pragma HLS STREAM variable=ciao_squeeze_stream2 depth=1024
#pragma HLS STREAM variable=ciao_squeeze_stream3 depth=1024
...
split_ciao(ciao_stream, ciao_done_stream, ciao_stream0, ciao_stream1, ciao_stream2, ciao_stream3,
