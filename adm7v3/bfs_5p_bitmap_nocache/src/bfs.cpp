...
#define BRAM_BANK_NUM 64

// 64 x 8K x 16bit bitmap memory
#define BITMAP_MEM_DEPTH (8*1024) 
...
#pragma HLS array_partition variable=bitmap cyclic factor=64 dim=1
...
//#pragma HLS DEPENDENCE variable=bitmap intra raw true
   	...
   	if(level == 0){
		...
		uint13_dt offset = root_vidx.range(16, 4);
		uint6_dt dim = root_vidx.range(22, 17);
...
squeeze_main:
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			uint13_dt offset = vidx.range(16, 4);
			uint6_dt dim = vidx.range(22, 17);
			uint16_dt mask = get_mask(n);
