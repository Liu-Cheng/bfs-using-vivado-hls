		...
		hls::stream<uint1_dt> &rpao_done_stream)
{
    ...
    #pragma HLS pipeline
		...
		if(frontier_empty != 1){
			...
			uint28_dt start_tag = vidx.range(31, 4);
			uint28_dt end_tag = vidx_plus1.range(31, 4);
			uint4_dt start_offset = vidx.range(3,0);
			uint4_dt end_offset = vidx_plus1.range(3, 0);
			int512_dt word_old = cache_data[0];
			uint28_dt _tag = tag[0];
