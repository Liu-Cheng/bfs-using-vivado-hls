...
// 64 x 8K x 16bit bitmap memory
#define BITMAP_MEM_DEPTH (4096) 
#define BITMAP_MEM_WIDTH 12
...
static void split_ciao(
		...
		hls::stream<int> &ciao_stream4,
		hls::stream<int> &ciao_stream5,
		hls::stream<int> &ciao_stream6,
		hls::stream<int> &ciao_stream7,
		...
		hls::stream<uint1_dt> &ciao_done_stream4,
		hls::stream<uint1_dt> &ciao_done_stream5,
		hls::stream<uint1_dt> &ciao_done_stream6,
		hls::stream<uint1_dt> &ciao_done_stream7
		)
{
	int done = 0;
	int ciao_empty = 0;
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			uint3_dt channel = vidx.range(8, 6);
			
			if(channel == 0) ciao_stream0 << vidx;
			if(channel == 1) ciao_stream1 << vidx;
			if(channel == 2) ciao_stream2 << vidx;
			if(channel == 3) ciao_stream3 << vidx;

			if(channel == 4) ciao_stream4 << vidx;
			if(channel == 5) ciao_stream5 << vidx;
			if(channel == 6) ciao_stream6 << vidx;
			if(channel == 7) ciao_stream7 << vidx;
		...
		)
{
	...
	static uint16_dt bitmap[BRAM_BANK_NUM][BITMAP_MEM_DEPTH];
...
//#pragma HLS DEPENDENCE variable=bitmap intra raw true
   	...
   	if(level == 0){
		...
		uint12_dt offset;
		offset.range(1, 0) = root_vidx.range(5, 4);
		offset.range(11, 2) = root_vidx.range(18, 9);
		uint3_dt channel = root_vidx.range(8, 6);
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			uint3_dt channel = vidx.range(8, 6);
			uint12_dt offset;
			offset.range(1, 0) = vidx.range(5, 4);
			offset.range(11, 2)	= vidx.range(18, 9);
			uint4_dt dim = vidx.range(22, 19);
			uint16_dt mask = get_mask(n);
			uint16_dt bv = bitmap[dim][offset];
			if((bv & mask) == 0){
				ciao_squeeze_stream << vidx;
				bitmap[dim][offset] = bv | mask;
			}
		}
...
void bfs(
		...
		char *depth_for_update3,
		char *depth_for_update4,
		char *depth_for_update5,
		char *depth_for_update6,
		char *depth_for_update7,
		const rpa_cache_dt *rpaoA, 
		const rpa_cache_dt *rpaoB, 
...
#pragma HLS INTERFACE m_axi port=depth_for_update0 offset=slave bundle=gmem20
#pragma HLS INTERFACE m_axi port=depth_for_update1 offset=slave bundle=gmem21
#pragma HLS INTERFACE m_axi port=depth_for_update2 offset=slave bundle=gmem22
#pragma HLS INTERFACE m_axi port=depth_for_update3 offset=slave bundle=gmem23
#pragma HLS INTERFACE m_axi port=depth_for_update4 offset=slave bundle=gmem24
#pragma HLS INTERFACE m_axi port=depth_for_update5 offset=slave bundle=gmem25
#pragma HLS INTERFACE m_axi port=depth_for_update6 offset=slave bundle=gmem26
#pragma HLS INTERFACE m_axi port=depth_for_update7 offset=slave bundle=gmem27
...
hls::stream<int> ciao_squeeze_stream4;
hls::stream<int> ciao_squeeze_stream5;
hls::stream<int> ciao_squeeze_stream6;
hls::stream<int> ciao_squeeze_stream7;

hls::stream<uint1_dt> squeeze_done_stream4;
hls::stream<uint1_dt> squeeze_done_stream5;
hls::stream<uint1_dt> squeeze_done_stream6;
hls::stream<uint1_dt> squeeze_done_stream7;
...
#pragma HLS STREAM variable=ciao_stream4 depth=32
#pragma HLS STREAM variable=ciao_stream5 depth=32
#pragma HLS STREAM variable=ciao_stream6 depth=32
#pragma HLS STREAM variable=ciao_stream7 depth=32
...
#pragma HLS STREAM variable=ciao_done_stream4 depth=4
#pragma HLS STREAM variable=ciao_done_stream5 depth=4
#pragma HLS STREAM variable=ciao_done_stream6 depth=4
#pragma HLS STREAM variable=ciao_done_stream7 depth=4

#pragma HLS STREAM variable=ciao_squeeze_stream0 depth=64
#pragma HLS STREAM variable=ciao_squeeze_stream1 depth=64
#pragma HLS STREAM variable=ciao_squeeze_stream2 depth=64
#pragma HLS STREAM variable=ciao_squeeze_stream3 depth=64

#pragma HLS STREAM variable=ciao_squeeze_stream4 depth=64
#pragma HLS STREAM variable=ciao_squeeze_stream5 depth=64
#pragma HLS STREAM variable=ciao_squeeze_stream6 depth=64
#pragma HLS STREAM variable=ciao_squeeze_stream7 depth=64
...
#pragma HLS STREAM variable=squeeze_done_stream4 depth=4
#pragma HLS STREAM variable=squeeze_done_stream5 depth=4
#pragma HLS STREAM variable=squeeze_done_stream6 depth=4
#pragma HLS STREAM variable=squeeze_done_stream7 depth=4
...
split_ciao(ciao_stream, ciao_done_stream, 
		ciao_stream0, ciao_stream1, ciao_stream2, ciao_stream3,
		ciao_stream4, ciao_stream5, ciao_stream6, ciao_stream7,
		ciao_done_stream0, ciao_done_stream1, ciao_done_stream2, ciao_done_stream3,
		ciao_done_stream4, ciao_done_stream5, ciao_done_stream6, ciao_done_stream7
		);

bfs_squeeze_ciao0(ciao_stream0, ciao_done_stream0, ciao_squeeze_stream0, 
		squeeze_done_stream0, root_vidx, level, 0);
bfs_squeeze_ciao1(ciao_stream1, ciao_done_stream1, ciao_squeeze_stream1, 
		squeeze_done_stream1, root_vidx, level, 1);
bfs_squeeze_ciao2(ciao_stream2, ciao_done_stream2, ciao_squeeze_stream2, 
		squeeze_done_stream2, root_vidx, level, 2);
bfs_squeeze_ciao3(ciao_stream3, ciao_done_stream3, ciao_squeeze_stream3, 
		squeeze_done_stream3, root_vidx, level, 3);

bfs_squeeze_ciao4(ciao_stream4, ciao_done_stream4, ciao_squeeze_stream4, 
		squeeze_done_stream4, root_vidx, level, 4);
bfs_squeeze_ciao5(ciao_stream5, ciao_done_stream5, ciao_squeeze_stream5, 
		squeeze_done_stream5, root_vidx, level, 5);
bfs_squeeze_ciao6(ciao_stream6, ciao_done_stream6, ciao_squeeze_stream6, 
		squeeze_done_stream6, root_vidx, level, 6);
bfs_squeeze_ciao7(ciao_stream7, ciao_done_stream7, ciao_squeeze_stream7, 
		squeeze_done_stream7, root_vidx, level, 7);

update_depth(depth_for_update0, ciao_squeeze_stream0, squeeze_done_stream0, level_plus1);
update_depth(depth_for_update1, ciao_squeeze_stream1, squeeze_done_stream1, level_plus1);
update_depth(depth_for_update2, ciao_squeeze_stream2, squeeze_done_stream2, level_plus1);
update_depth(depth_for_update3, ciao_squeeze_stream3, squeeze_done_stream3, level_plus1);

update_depth(depth_for_update4, ciao_squeeze_stream4, squeeze_done_stream4, level_plus1);
update_depth(depth_for_update5, ciao_squeeze_stream5, squeeze_done_stream5, level_plus1);
update_depth(depth_for_update6, ciao_squeeze_stream6, squeeze_done_stream6, level_plus1);
update_depth(depth_for_update7, ciao_squeeze_stream7, squeeze_done_stream7, level_plus1);
