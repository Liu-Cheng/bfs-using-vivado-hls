		...
		hls::stream<uint1_dt> &rpao_done_stream)
{
	...
	rpao_tag_dt tag[1] = {RPAO_TAG_INIT_VAL};
    ...
    #pragma HLS PIPELINE
		...
		if(frontier_empty != 1){
			...
			rpao_tag_dt req_tag = bank_vidx.range(31, RPAO_OFFSET_WIDTH);
			rpao_offset_dt req_offset = bank_vidx.range(RPAO_OFFSET_WIDTH - 1, 0);
			rpao_cache_dt _word = cache_data[0];
			rpao_tag_dt _tag = tag[0];
			if(req_tag == _tag){
				rpao_stream << _word.range((req_offset + 1)*64 - 1, req_offset * 64);
			}
			else{
				rpao_cache_dt word = rpao[req_tag];
				rpao_stream << word.range((req_offset + 1)*64 - 1, req_offset * 64);
				cache_data[0] = word;
				tag[0] = req_tag;
			}
		}
