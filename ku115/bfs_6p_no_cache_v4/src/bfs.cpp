        ...
        int vertex_num)
{
    ...
    read_depth: 
	for (uint32_dt i = 0; i < vertex_num; i += BUF_SIZE){
		...
		if(bank_idx == 0){
            ...
            #pragma HLS pipeline
				buffer[j] = depth_b0[bank_offset + j];
			}
		}
		else if(bank_idx == 1){
			for(int j = 0; j < BUF_SIZE; j++){
            #pragma HLS pipeline
				buffer[j] = depth_b1[bank_offset + j];
			}
		}
		else if(bank_idx == 2){
			for(int j = 0; j < BUF_SIZE; j++){
            #pragma HLS pipeline
				buffer[j] = depth_b2[bank_offset + j];
			}
		}
		else{
			for(int j = 0; j < BUF_SIZE; j++){
            #pragma HLS pipeline
				buffer[j] = depth_b3[bank_offset + j];
			}
		}
...
#pragma HLS STREAM variable=ngb_stream0 depth=1024
#pragma HLS STREAM variable=ngb_stream1 depth=1024
#pragma HLS STREAM variable=ngb_stream2 depth=1024
#pragma HLS STREAM variable=ngb_stream3 depth=1024
