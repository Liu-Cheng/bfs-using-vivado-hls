...
static void update_depth(
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			bank_offset.range(5, 0) = vidx.range(5, 0);
			bank_offset.range(30, 6) = vidx.range(31, 7);
			bank_offset.range(31, 31) = 0;
			char d = depth[bank_offset]; 
			if(d == -1) depth[bank_offset] = level + 1;
		}
