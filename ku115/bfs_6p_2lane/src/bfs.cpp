		...
		const char level)
{
    ...
    for (int i = 0; i < vertex_num; i++){
    #pragma HLS pipeline II=1
		char d = depth_inspect_char_stream.read();
		if(d == level){
			frontier_stream << i;
			counter++;
		}
...
//#pragma HLS DEPENDENCE variable=cache_tag inter false
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			...
			depth_rd_index_dt index = vidx.range(
					DEPTH_RD_OFFSET_WIDTH + DEPTH_RD_INDEX_WIDTH - 1, 
...
static void ngb_merger(
	...
	while(ngbA_empty != 1 || ngbB_empty != 1 || Adone != 1 || Bdone != 1){
...
//#pragma HLS DEPENDENCE variable=cache_dirty inter false
    ...
    #pragma HLS pipeline
		...
		if(ngb_empty != 1){
			...
			depth_wr_index_dt index = vidx.range(
					DEPTH_WR_OFFSET_WIDTH + DEPTH_WR_INDEX_WIDTH - 1, 
