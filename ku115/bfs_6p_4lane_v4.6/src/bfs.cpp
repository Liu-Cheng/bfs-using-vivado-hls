...
#define DEPTH_CACHE_SIZE 64
#define DEPTH_OFFSET_WIDTH 6
#define DEPTH_TAG_INIT_VAL 0x3FFFFFF
#define DEPTH_TAG_WIDTH (32 - DEPTH_OFFSET_WIDTH)
        ...
        int vertex_num)
{
	...
	int512_dt buffer2[SML_BUF_SIZE];
	int512_dt buffer3[SML_BUF_SIZE];

	int word_num = vertex_num >> 6;
    read_depth: 
	for (uint32_dt i = 0; i < word_num; i += 4 * SML_BUF_SIZE){
		uint32_dt bank_idx = i >> 2;
		int len = SML_BUF_SIZE;
		if(i + 4 * SML_BUF_SIZE > word_num) {
			len = (word_num - i) >> 2; 
		}
        ...
        #pragma HLS pipeline
			...
			buffer2[j] = depth_b2[bank_idx + j];
			buffer3[j] = depth_b3[bank_idx + j];
		}
...
static void merge_ciao(
		...
		hls::stream<int> &merged_ciao_b2_stream,
		hls::stream<int> &merged_ciao_b3_stream,
		hls::stream<uint1_dt> &merged_ciao_b0_done_stream,
		hls::stream<uint1_dt> &merged_ciao_b1_done_stream,
		hls::stream<uint1_dt> &merged_ciao_b2_done_stream,
		hls::stream<uint1_dt> &merged_ciao_b3_done_stream
		)
{
			...
			|| (done_b0 != 1 || done_b1 != 1 || done_b2 != 1 || done_b3 != 1))
	{
		...
		if(ciao_b0_empty != 1){
			...
			uint2_dt bank_sel = vidx.range(7, 6);
			if(bank_sel == 0){
			...
			if(bank_sel == 2){
				merged_ciao_b2_stream << vidx;
			}
			if(bank_sel == 3){
				merged_ciao_b3_stream << vidx;
			}
		}
	...
	merged_ciao_b2_done_stream << 1;
	merged_ciao_b3_done_stream << 1;
...
static void update_depth_s0(
	...
	int512_dt cache_data[1] ={0};
	depth_tag_dt cache_tag[1] = {DEPTH_TAG_INIT_VAL};
    ...
    #pragma HLS pipeline
		...
		if(ciao_empty != 1){
			uint32_dt vidx = ciao_stream.read();
			depth_tag_dt tag = vidx.range(31, DEPTH_OFFSET_WIDTH);
			depth_offset_dt offset = vidx.range(DEPTH_OFFSET_WIDTH - 1, 0);
			depth_tag_dt _tag = cache_tag[0];
			...
			char d;
			if(tag == _tag){
				d = cache_data[0].range((offset + 1) * 8 - 1, offset * 8);
			}
			else{
				int512_dt word = depth[tag]; 
...
static void update_depth_s1(
    ...
    #pragma HLS pipeline
		...
		if(update_empty != 1){
			...
			if(tag == _tag){
				cache_data[0].range((offset + 1) * 8 - 1, offset*8) = level + 1;
				cache_dirty[0] = 1;
			}
			else{
				int512_dt word = depth[tag];
	...
	if(cache_dirty[0] == 1) {
		depth[cache_tag[0]] = cache_data[0];
		cache_dirty[0] = 0;
	}
}
*/
...
void bfs(
		...
		int512_dt *depth_inspect_b2,
		int512_dt *depth_inspect_b3,

		char *depth_update_b0,
		char *depth_update_b1,
		char *depth_update_b2,
		char *depth_update_b3,
		...
		int512_dt *depth_read_b2,
		int512_dt *depth_read_b3,
		const int vertex_num,
		const char level
		)
{
...
#pragma HLS INTERFACE m_axi port=depth_inspect_b2 offset=slave bundle=gmem2
#pragma HLS INTERFACE m_axi port=depth_inspect_b3 offset=slave bundle=gmem3

#pragma HLS INTERFACE m_axi port=depth_update_b0 offset=slave bundle=gmem4
#pragma HLS INTERFACE m_axi port=depth_update_b1 offset=slave bundle=gmem5
#pragma HLS INTERFACE m_axi port=depth_update_b2 offset=slave bundle=gmem6
#pragma HLS INTERFACE m_axi port=depth_update_b3 offset=slave bundle=gmem7

#pragma HLS INTERFACE m_axi port=rpao_b0 offset=slave bundle=gmem8
#pragma HLS INTERFACE m_axi port=rpao_b1 offset=slave bundle=gmem9
#pragma HLS INTERFACE m_axi port=rpao_b2 offset=slave bundle=gmem10
#pragma HLS INTERFACE m_axi port=rpao_b3 offset=slave bundle=gmem11

#pragma HLS INTERFACE m_axi port=ciao_b0 offset=slave bundle=gmem12
#pragma HLS INTERFACE m_axi port=ciao_b1 offset=slave bundle=gmem13
#pragma HLS INTERFACE m_axi port=ciao_b2 offset=slave bundle=gmem14
#pragma HLS INTERFACE m_axi port=ciao_b3 offset=slave bundle=gmem15

#pragma HLS INTERFACE m_axi port=frontier_size offset=slave bundle=gmem16

#pragma HLS INTERFACE m_axi port=depth_read_b0 offset=slave bundle=gmem17
#pragma HLS INTERFACE m_axi port=depth_read_b1 offset=slave bundle=gmem18
#pragma HLS INTERFACE m_axi port=depth_read_b2 offset=slave bundle=gmem19
#pragma HLS INTERFACE m_axi port=depth_read_b3 offset=slave bundle=gmem20
...
hls::stream<int> squeezed_ciao_b2_stream;
hls::stream<int> squeezed_ciao_b3_stream;

hls::stream<uint1_dt> squeezed_ciao_b0_done_stream;
hls::stream<uint1_dt> squeezed_ciao_b1_done_stream;
hls::stream<uint1_dt> squeezed_ciao_b2_done_stream;
hls::stream<uint1_dt> squeezed_ciao_b3_done_stream;

hls::stream<int> vidx_update_b0_stream;
hls::stream<int> vidx_update_b1_stream;
hls::stream<int> vidx_update_b2_stream;
hls::stream<int> vidx_update_b3_stream;

hls::stream<uint1_dt> update_done_b0_stream;
hls::stream<uint1_dt> update_done_b1_stream;
hls::stream<uint1_dt> update_done_b2_stream;
hls::stream<uint1_dt> update_done_b3_stream;
...
#pragma HLS STREAM variable=merged_ciao_b2_stream depth=32
#pragma HLS STREAM variable=merged_ciao_b3_stream depth=32
...
#pragma HLS STREAM variable=merged_ciao_b2_done_stream depth=4
#pragma HLS STREAM variable=merged_ciao_b3_done_stream depth=4

#pragma HLS STREAM variable=squeezed_ciao_b0_stream depth=32
#pragma HLS STREAM variable=squeezed_ciao_b1_stream depth=32
#pragma HLS STREAM variable=squeezed_ciao_b2_stream depth=32
#pragma HLS STREAM variable=squeezed_ciao_b3_stream depth=32
...
#pragma HLS STREAM variable=squeezed_ciao_b2_done_stream depth=4
#pragma HLS STREAM variable=squeezed_ciao_b3_done_stream depth=4

#pragma HLS STREAM variable=vidx_update_b0_stream depth=32
#pragma HLS STREAM variable=vidx_update_b1_stream depth=32
#pragma HLS STREAM variable=vidx_update_b2_stream depth=32
#pragma HLS STREAM variable=vidx_update_b3_stream depth=32

#pragma HLS STREAM variable=update_done_b0_stream depth=4
#pragma HLS STREAM variable=update_done_b1_stream depth=4
#pragma HLS STREAM variable=update_done_b2_stream depth=4
#pragma HLS STREAM variable=update_done_b3_stream depth=4
...
merge_ciao(
		...
		merged_ciao_b2_stream,
		merged_ciao_b3_stream,
		merged_ciao_b0_done_stream,
		merged_ciao_b1_done_stream,
		merged_ciao_b2_done_stream,
		merged_ciao_b3_done_stream
		);
...
squeeze_ciao(
		merged_ciao_b2_stream,
		merged_ciao_b2_done_stream,
		squeezed_ciao_b2_stream,
		squeezed_ciao_b2_done_stream,
		level
		);
...
squeeze_ciao(
		merged_ciao_b3_stream,
		merged_ciao_b3_done_stream,
		squeezed_ciao_b3_stream,
		squeezed_ciao_b3_done_stream,
		level
		);
...
update_depth_s0(
		...
		squeezed_ciao_b2_stream,
		squeezed_ciao_b2_done_stream,
		vidx_update_b2_stream,
		update_done_b2_stream,
		level
		);
...
update_depth_s0(
		...
		squeezed_ciao_b3_stream,
		squeezed_ciao_b3_done_stream,
		vidx_update_b3_stream,
		update_done_b3_stream,
		level
		);
