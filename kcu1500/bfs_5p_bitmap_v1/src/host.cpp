...
int main(int argc, char **argv) {
	...
	int  *rpaInfo          = (int *)malloc(2 * vertexNum * sizeof(int));
	...
	xcl_set_kernel_arg(krnl_bfs, 2, sizeof(cl_mem), &devMemNextFrontier);
	xcl_set_kernel_arg(krnl_bfs, 3, sizeof(cl_mem), &devMemNextFrontierSize);
	xcl_set_kernel_arg(krnl_bfs, 5, sizeof(int),    &startVertexIdx);
	xcl_set_kernel_arg(krnl_bfs, 6, sizeof(int),    &segSize);
	xcl_set_kernel_arg(krnl_bfs, 7, sizeof(int),    &pad);
	...
	while(frontierSize > 0){
		...
		xcl_set_kernel_arg(krnl_bfs, 4, sizeof(int),    &frontierSize); 
		xcl_set_kernel_arg(krnl_bfs, 8, sizeof(char),   &level); 
