...
int main(int argc, char **argv) {
	...
	for(int i = 0; i < pad; i++){
		nextFrontier[i]    = (int *)malloc(segSize * sizeof(int));
	}
	...
	xcl_set_kernel_arg(krnl_bfs, 18, sizeof(cl_mem), &devMemNextFrontierSize);
	xcl_set_kernel_arg(krnl_bfs, 20, sizeof(int),    &startVertexIdx);
	xcl_set_kernel_arg(krnl_bfs, 21, sizeof(int),    &pad);
	...
	while(frontierSize > 0){
		...
		xcl_set_kernel_arg(krnl_bfs, 19, sizeof(int),    &frontierSize); 
		xcl_set_kernel_arg(krnl_bfs, 22, sizeof(char),   &level); 
